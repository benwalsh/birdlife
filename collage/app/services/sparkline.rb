...
class Sparkline
  ...
  Paths = Data.define(:path, :fill, :w, :h)

  class << self
    # counts: up to 24 hourly buckets (oldest first). Returns the stroke path (the
    # curve) and the fill path (same curve closed down to the baseline).
    def paths(counts, width: W, height: H, pad: PAD)
      counts = Array(counts).map(&:to_f)
      return flat(width, height, pad) if counts.length < 2 || counts.sum.zero?

      points = plot_points(smooth(counts), width, height, pad)
      line = spline(points, width, height)
      fill = "#{line} L #{r(points.last[0])} #{height} L #{r(points.first[0])} #{height} Z"
      Paths.new(path: line, fill: fill, w: width, h: height)
    end

    private

    # A gentle resting line low on the card — the honest picture of a quiet day.
    def flat(width, height, pad)
      y = height - pad - ((height - (2 * pad)) * 0.12)
      path = "M #{pad} #{r(y)} L #{width - pad} #{r(y)}"
      Paths.new(path: path, fill: "#{path} L #{width - pad} #{height} L #{pad} #{height} Z", w: width, h: height)
    end
