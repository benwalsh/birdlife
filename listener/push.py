...
so culfinbirds.net mirrors what the wall has heard AND whether the mic is alive. The
...
Run standalone (`python listener/push.py`, or birdlife-push.timer), or called every few
...
def sync(dry_run: bool = False) -> None:
    ...
    try:
        detections = push_stream(
            con, table='detections', columns=COLUMNS, key_columns=KEY_COLUMNS,
            cursor=DETECTION_CURSOR, url=url, token=token, payload_key='detections', dry_run=dry_run,
        )
        ticks = push_stream(
            con, table='heartbeats', columns=HEARTBEAT_COLUMNS, key_columns=HEARTBEAT_COLUMNS,
            cursor=HEARTBEAT_CURSOR, url=heartbeats_url(url), token=token,
            payload_key='heartbeats', dry_run=dry_run,
        )
    finally:
        con.close()
    if not dry_run:
