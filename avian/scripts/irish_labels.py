...
still missing its Irish name, e.g. Mallard, isn't dropped from a restyle).
    ...
    uv run python avian/scripts/irish_labels.py            # -> Sci|Com lines
    uv run python avian/scripts/irish_labels.py | wc -l    # how many
"""
...
def main() -> int:
    ...
    db = ROOT / "collage/storage/development.sqlite3"
    if db.exists():
