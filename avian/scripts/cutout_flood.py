...
than BiRefNet matting, but at collage/atlas size it reads clean. Already-cut
...
the flecks from an existing cutout, non-destructively).
    ...
    uv run python avian/scripts/cutout_flood.py             # cut uncut illustrations
    uv run python avian/scripts/cutout_flood.py --force     # re-cut everything
    uv run python avian/scripts/cutout_flood.py --declutter # sweep flecks from existing cutouts
"""
...
DIR = Path(__file__).resolve().parents[1] / "assets" / "illustrations"
...
def main() -> int:
    ...
    ap.add_argument("--declutter", action="store_true",
                    help="sweep stray flecks from existing cutouts (non-destructive)")
    args = ap.parse_args()
