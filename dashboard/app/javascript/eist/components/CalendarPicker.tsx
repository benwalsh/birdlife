...
export function CalendarPicker({
  ...
  return (
    <div className="cal" role="group" aria-label={t('Choose a day', 'Roghnaigh lá')}>
      <button className="cal-arrow" onClick={() => step(-1)} disabled={atStart}
              aria-label={t('Previous day', 'Lá roimhe')}>
        <i className="ti ti-chevron-left" aria-hidden="true" />
      </button>
      <input className="cal-date" type="date" value={date} min={first} max={last}
             onChange={(e) => e.target.value && onChange(e.target.value)} />
      <button className="cal-arrow" onClick={() => step(1)} disabled={atEnd}
